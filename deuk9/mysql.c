...
if (!mysql_real_connect(&mysql, "localhost", "root", "159357", "test", 0, (char *)NULL, 0)) {
printf("MySQL Error %d: %s\n", mysql_errno(&mysql), mysql_error(&mysql));
exit(1);
}
while(1){
	...
	switch(choice){
		...
		case 2:
			...
			printf("\nADDRESS : ");
			scanf("%s", address);
			sprintf(query, "insert into lab values(%d, \'%s\', \'%s\')", id, tell, address);
			if (mysql_query(&mysql, query)) {
				printf("MySQL Error %d: %s\n", mysql_errno(&mysql), mysql_error(&mysql));
				exit(1);
			}
		...
		case 3:
			...
			printf("\nADDRESS : ");
			scanf("%s", address);
			sprintf(query, "update lab set tell='%s', address='%s' where id='%d'", tell, address, id);
			if (mysql_query(&mysql, query)) {
				printf("MySQL Error %d: %s\n", mysql_errno(&mysql), mysql_error(&mysql));
				exit(1);
			}
